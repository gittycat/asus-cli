...
Written by `asus wifi wps|security|country`. All reads below were observed on
...
WPS on, so `asus wifi wps` writes both — otherwise the radio and the web UI
...
asynchronous with no completion signal, which is why `asus firmware info`
sleeps before re-reading; the upgrade reports nothing at all, which is why
`asus firmware upgrade` says "requested" and points at `asus info`.

`asus firmware info` runs the check every time rather than reporting the
...
```bash
asus --json nvram vts_rulelist wl0.1_bss_enabled fw_enable_x
```

`asus nvram` has deliberately no write counterpart. Blind nvram writes are
