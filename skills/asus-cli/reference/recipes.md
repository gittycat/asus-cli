...
```bash
asus pf add --name Web --port 8080 --to-ip 192.168.50.30 --to-port 80 --yes
```
...
asus pf remove --name Plex --yes
```

Removing by external port works too: `asus pf remove --port 32400 --yes`.
...
asus parental enable --yes   # turns the whole feature on
```
...
```bash
asus wifi wps disable                    # dry run
asus wifi wps disable --yes
```

## "Move my WiFi to WPA3"

```bash
asus wifi security --band both --mode wpa2wpa3          # dry run
asus wifi security --band both --mode wpa2wpa3 --yes
```
...
```bash
asus wifi security --band 2ghz --mode wpa2wpa3 --mfp disabled --yes
```

## "Fix the 5 GHz country code"

```bash
asus nvram wl0_country_code wl1_country_code reg_spec location_code
...
```bash
asus firmware info             # asks the router to query ASUS, then reports
asus firmware info --notes     # same, plus the release note
asus firmware info --cached    # skip the check, report the stored value
```
...
asus firmware upgrade --yes --to 3.0.0.4.388.34098_g9b0c9ae
```
